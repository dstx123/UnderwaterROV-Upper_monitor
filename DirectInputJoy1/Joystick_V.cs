    ...
    public class Joystick_V : IDisposable
    {
        ...
        private void OnTimerCallback(object state)
        {
            ...
            if (result == JoystickAPI.JOYERR_NOERROR)
            {
                ...
                GetXYZButtons(infoEx.dwXpos, infoEx.dwYpos, infoEx.dwZpos, infoEx.dwRpos, infoEx.dwUpos ,infoEx .dwVpos , ref buttons);
        ...
        private JoystickButtons GetButtons(int dwButtons)
        {
            ...
            if ((dwButtons & JoystickAPI.JOY_BUTTON11) == JoystickAPI.JOY_BUTTON11)
            {
                buttons |= JoystickButtons.B11;
            }
            if ((dwButtons & JoystickAPI.JOY_BUTTON12) == JoystickAPI.JOY_BUTTON12)
            {
                buttons |= JoystickButtons.B12;
            }
        ...
        /// <summary>
        /// 获取X,Y,Z轴的状态，可通过不等式来改变手柄数值大小
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="buttons"></param>
        private void GetXYZButtons(int x, int y, int z, int r,int u, int v,ref JoystickButtons buttons)
        {
            //处理X,Y轴
        ...
        private void GetPOVButtons(int pov, ref JoystickButtons buttons)
        {
            ...
            if (value > 359) return;
            else if (value > 22 && value < 67) buttons |= JoystickButtons.RightUp;
            else if (value >= 67 && value < 112) buttons |= JoystickButtons.POV_Right;
            else if (value >= 112 && value < 157) buttons |= JoystickButtons.RightDown;
            else if (value >= 157 && value < 202) buttons |= JoystickButtons.POV_Down;
            else if (value >= 202 && value < 247) buttons |= JoystickButtons.LeftDown;
            else if (value >= 247 && value < 292) buttons |= JoystickButtons.POV_Left;
            else if (value >= 292 && value < 337) buttons |= JoystickButtons.LeftUp;
